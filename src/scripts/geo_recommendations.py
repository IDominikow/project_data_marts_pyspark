...
def main():
    ...
    geo_users_full_deduplicated = gtu.calculate_user_pairs_by_conditions(geo_events)   #Рассчитываем DataFrame уникальных пар юзеров без сообщений и с общими каналами
    ...
    geo_recommendations = geo_users_full_deduplicated\
        ...
        .filter(F.col('zone_id_1')==F.col('zone_id_2'))\
    ...
    geo_recommendations.write.mode("overwrite").parquet(f"{output_base_path}date={date}") # Записываем результат в analytics
